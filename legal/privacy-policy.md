...
Our backend also stores this identifier in a small key‑value database
(**Upstash**) as part of two short markers used to keep billing honest:

- one recorded when your one‑time welcome gems are issued, so they cannot be
  claimed twice;
- one recorded per analysis you run, so that if the app has to retry a request
  (for example, your connection dropped while the result was coming back) you
  are **not charged twice for the same analysis**.

These markers hold only the identifier, the type of request, and a random run
number. They contain **no photos, no generated text, no statistics, and no
personal details** — the value stored is literally the digit `1`. Retention is
...
| **Upstash** | Key‑value database used to prevent duplicate charges and duplicate welcome grants | Your random app‑user identifier and a random run number — nothing else |
...
- **Billing markers** (Section 3.4): the per‑analysis marker that stops a retry
  being charged twice is deleted automatically after **48 hours**. The marker
  recording that your one‑time welcome gems were issued is kept **indefinitely**
  — deleting it would let the same grant be claimed again, which is the whole
  reason it exists. Both hold only identifiers, never content.
