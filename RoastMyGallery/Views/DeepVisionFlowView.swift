            ...
            Toggle(isOn: $hasConsented) {
                VStack(alignment: .leading, spacing: Theme.Spacing.xs) {
                    Text("Send these photos for analysis")
                        .font(Theme.Typography.headline)
                    Text("The \(runner.selection.count) photos you picked — and only those — will be resized on your device and sent once to Google Gemini, a third-party AI service, to be read. They are not used to train any AI model. If you leave this off, no photo is uploaded. We don't keep a copy.")
                        .font(Theme.Typography.caption)
                        .foregroundStyle(Theme.Colors.textSecondary)
                        .lineSpacing(2)
                }
